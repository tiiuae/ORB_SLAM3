        ...
        else
        {
            ...
            if (dist > 1)
            {
                ...
                Verbose::PrintMess("--Number of observations: " + to_string(numMonoOptPoints) + " in mono and " +
                                       to_string(numStereoOptPoints) + " in stereo",
                                   Verbose::VERBOSITY_DEBUG);
...
int Optimizer::PoseOptimization(Frame* pFrame)
{
    ...
    // cout << "[PoseOptimization]: initial correspondences-> " << nInitialCorrespondences << " --- outliers-> " << nBad
    // << endl;
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, vector<KeyFrame*>& vpNonEnoughOptKFs)
{
    ...
    // Verbose::PrintMess("LM-LBA: There are " + to_string(lLocalKeyFrames.size()) + " KFs and " +
    // to_string(lLocalMapPoints.size()) + " MPs to optimize. " + to_string(num_fixedKF) + " KFs are fixed",
    // Verbose::VERBOSITY_DEBUG);
    ...
    // std::cout << "LBA time = " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() <<
    // "[ms]" << std::endl; std::cout << "Keyframes: " << nKFs << " --- MapPoints: " << nPoints << " --- Edges: " <<
    // nEdges << endl;
    ...
    if (bDoMore)
    {
        ...
        Verbose::PrintMess("LM-LBA: First optimization has " + to_string(nMonoBadObs) + " monocular and " +
                               to_string(nStereoBadObs) + " stereo bad observations",
                           Verbose::VERBOSITY_DEBUG);
    ...
    if (!vToErase.empty())
    {
        ...
        // cout << "LM-LBA: There are " << vToErase.size() << " observations whose will be deleted from the map" <<
        // endl;
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF)
{
    ...
    // to_string(vpMapPointEdgeMono.size()+vpMapPointEdgeStereo.size()), Verbose::VERBOSITY_DEBUG);
                                           ...
                                           double scale)
{
    ...
    for (KeyFrame* pKFi : vpNonFixedKFs)
    {
        ...
        pKFi->GetVectorCovisibleKeyFrames().size() << " KFs" << endl;
            ...
            string namefile = "./test_OptEssent/Essent_" + to_string(pCurKF->mnId) + "_KF" + to_string(pKFi->mnId) +"_D"
    ...
    for (MapPoint* pMPi : vpNonCorrectedMPs)
    {
        ...
        // Converter::toMatrix3d(TNonCorrectedwr.rowRange(0,3).colRange(0,3)); Eigen::Matrix<double,3,1> tNonCorrectedwr
                                      ...
                                      bool* pbStopFlag)
{
    ...
    // Verbose::PrintMess("LBA: number total of edged -> " + to_string(vpEdgeKFMono.size() + vpEdgeKFStereo.size()),
    // Verbose::VERBOSITY_NORMAL);
        ...
        if (dist > 1.0)
        {
            if (bShowImages)
            {
                ...
                for (int i = 0; i < vpMonoMPsOpt.size(); ++i)
                {
                    ...
                    if (index < 0)
                    {
                        // cout << "LBA ERROR: KF has a monocular observation which is not recognized by the MP" <<
                        // endl; cout << "LBA: KF " << pKFi->mnId << " and MP " << vpMonoMPsOpt[i]->mnId << " with index
                        // " << endl;
                        continue;
                    }
                ...
                std::string namefile = "./test_LBA/LBA_KF" + to_string(pKFi->mnId) + "_" +
