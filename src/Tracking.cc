      ...
      mpCamera2(nullptr)
{
    ...
    D_r.empty()
        ...
        else
        {
            ...
            // M1r y M2r son las matrices relativas al mapeo correspondiente a la rectificación de mapa en el eje X e Y
    respectivamente
    ...
    f_track_times
        << "# ORB_Ext(ms), Stereo matching(ms), Preintegrate_IMU(ms), Pose pred(ms), LocalMap_track(ms), NewKF_dec(ms)"
        << endl;
    f_track_times << fixed;
#endif
}
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if (!mbOnlyTracking)
        {
            ...
            mTime_PosePred = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(timeEndPosePredict -
                                                                                                   timeStartPosePredict)
                                 .count();
#endif
        }
        else
        ...
        if (!mbOnlyTracking)
        {
            if (bOK)
            {
                ...
                mTime_LocalMapTrack = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
        ...
        if (bOK || mState == RECENTLY_LOST)
        {
            ...
            mTime_NewKF_Dec =
                std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(timeEndNewKF - timeStartNewKF)
                    .count();
#endif
