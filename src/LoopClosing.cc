            ...
            if (NewDetectCommonRegions())
            {
                if (mbMergeDetected)
                {
                    ...
                    else
                    {
                        ...
                        //                        cout << "Angle Rw2w1: " <<
                        //                        180*LogSO3(mSold_new.rotation().toRotationMatrix())/3.14 << endl; cout
                        //                        << "scale w2w1: " << mSold_new.scale() << endl;
...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    // cout << "LC: bMergeDetectedInKF: " << bMergeDetectedInKF << "   bLoopDetectedInKF: " << bLoopDetectedInKF <<
    // endl;
                                                 ...
                                                 std::vector<MapPoint*>& vpMatchedMPs)
{
    ...
    if (nNumProjMatches >= nProjMatches)
    {
        ...
        if (numOptMatches > nProjOptMatches)
        {
            ...
            // cout << "REFFINE-SIM3: Projection with optimize Sim3 from last KF with " << nNumProjMatches << " matches"
            // << endl;
                    ...
                    if (numOptMatches >= nSim3Inliers)
                    {
                        ...
                        if (numProjOptMatches >= nProjOptMatches)
                        {
                            ...
                            // for(int j=0; j<vpCurrentCovKFs.size(); ++j)
                            {
                                ...
                                if (bValid)
                                {
                                    // cout << "BoW: KF " << pKFj->mnId << " has " << numProjMatches_j << " matches" <<
                                    // endl;
                                    ...
                                    cout << "BoW: KF " << pKFi->mnId << " to KF " << pKFj->mnId << " is separated by "
                                         << dist << " meters" << endl;
                                    ...
                                    cout << "BoW: Rotation in angles (x, y, z) -> (" << v_euler[0] << ", " << v_euler[1]
                                         << ", " << v_euler[2] << ")" << endl;
                                    nNumKFs++;
                                                ...
                                                std::vector<MapPoint*>& vpMatchedMPs)
{
    ...
    if (nNumProjMatches >= nProjMatches)
    {
        /*cout << "PR_From_LastKF: KF " << pCurrentKF->mnId << " has " << nNumProjMatches << " with KF " <<
        pMatchedKF->mnId << endl;
        ...
        // cout << "PR_From_LastKF: Coord in X -> " << min_x << ", " << max_x << "; and Y -> " << min_y << ", " << max_y
        // << endl; cout << "PR_From_LastKF: features area in X -> " << (max_x - min_x) << " and Y -> " << (max_y -
        // min_y) << endl;
...
void LoopClosing::MergeLocal()
{
    ...
    Verbose::PrintMess("MERGE-VISUAL: Initially there are " + to_string(pCurrentMap->KeyFramesInMap()) + " KFs and " +
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Initial number of KFs in local window from active map: " + to_string(spLocalWindowKFs.size()),
        Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Last number of KFs in local window from the active map: " + to_string(spLocalWindowKFs.size()),
        Verbose::VERBOSITY_DEBUG);
    ...
    if (mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO)
    {
        ...
        Optimizer::MergeInertialBA(
            mpLocalMapper->GetCurrKF(), mpMergeMatchedKF, &bStop, mpCurrentKF->GetMap(), vCorrectedSim3);
    }
    else
    {
    ...
    else
    {
...

        {
            ...
            Verbose::PrintMess("MERGE-VISUAL: There are " + to_string(pMergeMap->KeyFramesInMap()) + " KFs in the map",
                               Verbose::VERBOSITY_DEBUG);
            Verbose::PrintMess(
            ...
            Verbose::PrintMess("MERGE-VISUAL: There are " + to_string(pMergeMap->MapPointsInMap()) + " MPs in the map",
                               Verbose::VERBOSITY_DEBUG);
            Verbose::PrintMess(
...
void LoopClosing::MergeLocal2()
{
    ...
    cout << "MergeMap init ID: " << pMergeMap->GetInitKFid() << "       CurrMap init ID: " << pCurrentMap->GetInitKFid()
         << endl;
